...
class TemplateMatcher:
    ...
    def _ncc(self, P, T, mu_T, sig_T):
        ...
        if sig_P == 0 or sig_T == 0:  # Guard against division by zero
            return 0

        standard_P = (P - mu_P) / sig_P
        standard_T = (T - mu_T) / sig_T
    ...
    def run(self, search, start_x, start_y):
        ...
        while moving_center_x <= search_width - self.template_width // 2 - 1:
            while moving_center_y <= search_height - self.template_height // 2 - 1:
                # Get patch
                patch = self._get_patch(search, self.template_width // 2, self.template_height // 2, (moving_center_x, moving_center_y))
